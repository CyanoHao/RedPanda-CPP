 ...
 *
 * Breeze authors:
 *   Copyright (C) 2020-2021 Noah Davis <noahadvs@gmail.com>
                                ...
                                qreal lightnessAdjustment = 0.0)
{
    ...
    drawArrow(painter, rect, type, brush);
}
...
void Style::drawPrimitive(PrimitiveElement elem,
                              ...
                              const QWidget* widget) const
{
    ...
    case PE_Frame: {
        ...
        fillRectOutline(painter, option->rect, 1, swatch.color(S_frame_outline));
        break;
    }
    case PE_FrameMenu: {
        break;
    ...
    case PE_FrameGroupBox: {
        ...
        if (isFlat) {
            fillRectEdges(painter, frame, Qt::TopEdge, 1, swatch.color(S_window_divider));
        } else {
            paintBorderedRoundRect(painter, frame, GroupBox_Rounding, swatch, S_frame_outline, S_none);
        }
        break;
    }
    case PE_IndicatorBranch: {
        ...
        drawArrow(painter, r, arrow, swatch.brush(color));
        break;
    }
    case PE_IndicatorMenuCheckMark: {
        ...
        qreal dimx = dim * insetScale * CheckMark_WidthOfHeightScale;
        qreal dimy = dim * insetScale;
        QRectF r_(rx + (rw - dimx) / 2, ry + (rh - dimy) / 2, dimx, dimy);
        drawCheck(painter, d->checkBox_pen_scratch, r_, swatch, fgColor);
        break;
    }
    case PE_PanelTipLabel: {
        const auto &palette = option->palette;
        const auto &background = palette.color(QPalette::ToolTipBase);
        const qreal radius = DefaultFrame_Radius;
        painter->save();
        painter->setRenderHint(QPainter::Antialiasing);
    ...
    case PE_FrameTabBarBase: {
        ...
        fillRectEdges(painter, option->rect, edge, 1, swatch.color(S_frame_outline));
        ...
        fillRectEdges(painter, expandRect(option->rect, edge, -1), edge, 1, swatch.color(S_tabFrame_specular));
        break;
    }
    case PE_PanelScrollAreaCorner: {
        ...
        fillRectEdges(painter, option->rect, edges, 1, swatch.color(S_window_outline));
        break;
    }
    ...
    case PE_IndicatorArrowLeft: {
        ...
        int ax = (rw - aw) / 2 + rx;
        int ay = (rh - ah) / 2 + ry;
        drawArrow(painter, QRect(ax, ay, aw, ah), arrow, swatch);
        break;
    }
    ...
    case PE_IndicatorHeaderArrow: {
        ...
        if (!header)
            return;
        QRect r = header->rect;
        QPoint offset = QPoint(HeaderSortIndicator_HOffset, HeaderSortIndicator_VOffset);
        qreal lightness = DeriveColors::hack_isLightPalette(widget->palette()) ? 0.03 : 0.0;
        if (header->sortIndicator & QStyleOptionHeader::SortUp) {
            drawArrow(painter, r.translated(offset), Qt::DownArrow, swatch, true, lightness);
        } else if (header->sortIndicator & QStyleOptionHeader::SortDown) {
            drawArrow(painter, r.translated(offset), Qt::UpArrow, swatch, true, lightness);
        }
        break;
    }
    ...
    case PE_IndicatorToolBarSeparator: {
        ...
        if (option->state & State_Horizontal) {
            ...
            fillRectEdges(painter, r, Qt::RightEdge, 1, swatch.color(S_window_divider));
        } else {
    ...
    case PE_PanelButtonTool: {
        ...
        const qreal rounding = ToolButton_Rounding;
        ...
        QRect r = option->rect;
        PSave save(painter);
        paintBorderedRoundRect(painter, r, rounding, swatch, outline, fill);
        paintBorderedRoundRect(painter, r.adjusted(1, 1, -1, -1), rounding, swatch, specular, S_none);
        break;
    }
    ...
    case PE_PanelLineEdit: {
        ...
        painter->setRenderHint(QPainter::Antialiasing);
        painter->setPen(swatch.pen(SwatchColors::S_none));
        QColor bgColor = swatch.color(S_base);
        bgColor.setAlpha(100);
        painter->setBrush(bgColor);
        painter->drawRoundedRect(option->rect, LineEdit_Rounding, LineEdit_Rounding);

        // paintSolidRoundRect(painter, option->rect, LineEdit_Rounding, swatch, S_base);
        save.restore();
    ...
    case PE_IndicatorCheckBox: {
        ...
        if (!isFlat) {
            QRect fillR = r;
            fillRectOutline(painter, fillR, 1, swatch.color(outlineColor));
            fillR.adjust(1, 1, -1, -1);
            if (IndicatorShadows && !isPressed && isEnabled) {
                fillRectEdges(painter, fillR, Qt::TopEdge, 1, swatch.color(S_base_shadow));
        ...
        if (checkbox->state & State_NoChange) {
            ...
            QRectF r_(rx + (rw - dimx) / 2, ry + (rh - dimy) / 2, dimx, dimy);
            drawHyphen(painter, d->checkBox_pen_scratch, r_, swatch, fgColor);
        } else if (checkbox->state & State_On) {
            const qreal insetScale = 0.8;
            qreal rx, ry, rw, rh;
            QRectF(r.adjusted(1, 1, -1, -1)).getRect(&rx, &ry, &rw, &rh);
            // kinda wrong, assumes we're already square, but we probably are
            qreal dimx = rw * insetScale * CheckMark_WidthOfHeightScale;
            qreal dimy = rh * insetScale;
            QRectF r_(rx + (rw - dimx) / 2, ry + (rh - dimy) / 2, dimx, dimy);
            drawCheck(painter, d->checkBox_pen_scratch, r_, swatch, fgColor);
        }
    ...
    case PE_IndicatorRadioButton: {
        ...
        if (IndicatorShadows && !isSunken && isEnabled) {
    ...
    case PE_FrameFocusRect: {
            ...
            if (auto itemView = qobject_cast<const QAbstractItemView*>(widget)) {
                ...
                if (option->state & QStyle::State_Selected) {
                    ...
                    if (showCurrent) {
                        ...
                        fillRectOutline(painter, option->rect, thickness, borderColor);
                    }
                } else {
                    fillRectOutline(painter, option->rect, 1, swatch.color(S_highlight_outline));
                }
                break;
            }
        }
        ...
        PSave save(painter);
        paintBorderedRoundRect(
            painter, option->rect, FrameFocusRect_Rounding, swatch, S_highlight_outline, S_none);
        break;
    }
    ...
    case PE_PanelButtonBevel: {
        ...
        Q_UNUSED(isEnabled);
        bool hasFocus = (option->state & State_HasFocus && option->state & State_KeyboardFocusChange);
        const qreal rounding = PushButton_Rounding;
        Swatchy outline = S_window_outline;
        Swatchy fill = S_button;
        Swatchy specular = S_button_specular;
        if (isDown) {
    ...
    case PE_FrameTabWidget: {
        ...
        if (!twf)
            break;
        fillRectOutline(painter, option->rect, 1, swatch.color(S_frame_outline));
        fillRectOutline(painter, bgRect, 1, swatch.color(S_tabFrame_specular));
        break;
    }
    ...
    case Phantom_PE_IndicatorTabNew: {
        ...
        PSave save(painter);
        if (bg) {
            paintSolidRoundRect(painter, option->rect, PushButton_Rounding, swatch, bg);
        }
    ...
    case PE_PanelMenu: {
        ...
        // fillRectOutline(painter, option->rect, 1, swatch.color(S_window_divider));
        ...
        painter->drawRoundedRect(frameRect, DefaultFrame_Radius, DefaultFrame_Radius);
    ...
    case Phantom_PE_ScrollBarSliderVertical: {
        ...
        fillRectEdges(painter, edgeRect, edges, 1, swatch.color(S_window_outline));
        painter->fillRect(mainRect, swatch.color(thumbFill));
        fillRectOutline(painter, mainRect, 1, swatch.color(thumbSpecular));
        break;
    }
                            ...
                            const QWidget* widget) const
{
    ...
    case CE_Splitter: {
        ...
        if (r.width() < 5 || r.height() < 5)
            break;
        int length = SplitterMaxLength;
        // int thickness = dpiScaled(1);
        qreal radius = 0;
        QSize size;
        if (option->state & State_Horizontal) {
            if (r.height() < length)
        ...
        paintSolidRoundRect(painter, filledRect, radius, swatch, S_button_specular);
        // fillRectOutline(painter, filledRect.adjusted(-1, 0, 1, 0), 1, swatch.color(S_window_divider));
        break;
    }
    ...
    case CE_ToolBar: {
        ...
        if (isFloating) {
            fillRectOutline(painter, option->rect, 1, swatch.color(S_window_outline));
        }
        break;
    }
    ...
    case CE_HeaderSection: {
        ...
        QRect bgRect = expandRect(rect, edges, -1);
        painter->fillRect(bgRect, swatch.color(S_window));
        fillRectEdges(painter, rect, edges, 1, swatch.color(S_frame_outline));
        break;
    }
    case CE_HeaderLabel: {
        auto header = qstyleoption_cast<const QStyleOptionHeader*>(option);
        ...
        if (drawHighlightLine) {
            ...
            fillRectEdges(painter, r, edge, 1, swatch.color(S_itemView_headerOnLine));
        }
        break;
    }
    case CE_ProgressBarGroove: {
        const qreal rounding = ProgressBar_Rounding;
        QRect rect = option->rect;
        PSave save(painter);
        paintBorderedRoundRect(painter, rect, rounding, swatch, S_window_outline, S_base);
        save.restore();
        if (OverhangShadows && option->state & State_Enabled) {
            // Inner shadow
    ...
    case CE_ProgressBarContents: {
        ...
        progressBarFillRects(bar, filled, nonFilled, isIndeterminate);
        if (isIndeterminate || bar->progress > bar->minimum) {
            PSave save(painter);
            paintBorderedRoundRect(painter, filled, rounding, swatch, S_progressBar_outline, S_progressBar);
            paintBorderedRoundRect(
    ...
    case CE_MenuBarItem: {
        ...
        if (MenuBarDrawBorder && !isSelected) {
            fillRectEdges(painter, r, Qt::BottomEdge, 1, swatch.color(S_window_divider));
        }
        break;
    }
    ...
    case CE_MenuItem: {
        ...
        if (!menuItem)
            break;
        const auto metrics = MenuItemMetrics::ofFontHeight(option->fontMetrics.height());
        ...
        if (isSelected) {
            ...
            qreal item_radius = DefaultFrame_Radius / 2;
            painter->save();
            painter->setPen(Qt::NoPen);
        ...
        if (isCheckable) {
            ...
            QRect checkRect = menuItemCheckRect(metrics, option->direction, itemRect, hasSubMenu);
            ...
            if (menuItem->checkType & QStyleOptionMenuItem::Exclusive) {
                ...
                if (isChecked) {
                    drawCheck(painter, d->checkBox_pen_scratch, checkRect, swatch, signColor);
                }
            }
        }
        ...
        if (hasIcon) {
            QRect iconRect = menuItemIconRect(metrics, option->direction, itemRect, hasSubMenu);
        ...
        if (!s.isEmpty()) {
            QRect textRect =
                menuItemTextRect(metrics, option->direction, itemRect, hasSubMenu, hasIcon, menuItem->tabWidth);
            ...
            if (t >= 0) {
                QRect mnemonicR =
                    menuItemMnemonicRect(metrics, option->direction, itemRect, hasSubMenu, menuItem->tabWidth);
        ...
        if (hasSubMenu) {
            ...
            QRect arrowRect = menuItemArrowRect(metrics, option->direction, itemRect);
            Swatchy arrowColor = isSelected ? S_highlightedText : S_indicator_current;
            drawArrow(painter, arrowRect, arrow, swatch.brush(arrowColor));
        }
        painter->restore();
        break;
    }
    ...
    case CE_MenuBarEmptyArea: {
        QRect rect = option->rect;
        if (MenuBarDrawBorder) {
            fillRectEdges(painter, rect, Qt::BottomEdge, 1, swatch.color(S_window_divider));
        }
    ...
    case CE_TabBarTabShape: {
        ...
        const qreal rounding = TabBarTab_Rounding;
        ...
        Qt::Edge innerEdge = oppositeEdge(outerEdge);
        Qt::Edge edgeAwayNextTab = oppositeEdge(edgeTowardNextTab);
        QRect shapeClipRect = expandRect(option->rect, innerEdge, -2);
        QRect drawRect = expandRect(shapeClipRect, innerEdge, 3 + 2 * rounding + 1);
        if (!onlyOne && !lastTab) {
            drawRect = expandRect(drawRect, edgeTowardNextTab, tabOverlap);
            shapeClipRect = expandRect(shapeClipRect, edgeTowardNextTab, tabOverlap);
        }
        if (!isSelected) {
            int offset = proxy()->pixelMetric(PM_TabBarTabShiftVertical, option, widget);
            drawRect = expandRect(drawRect, outerEdge, -offset);
        }
        painter->save();
        ...
        if (hasFrame) {
            ...
            if (isSelected) {
                ...
                specular = TabBar_InactiveTabsHaveSpecular ? S_inactiveTabYesFrame_specular : S_none;
            }
        } else {
            ...
            if (isSelected) {
                ...
                specular = TabBar_InactiveTabsHaveSpecular ? S_inactiveTabNoFrame_specular : S_none;
            }
        }
        auto frameColor = isSelected ? S_frame_outline : S_window_outline;
        paintBorderedRoundRect(painter, drawRect, rounding, swatch, frameColor, thisFillColor);
        paintBorderedRoundRect(painter, drawRect.adjusted(1, 1, -1, -1), rounding, swatch, specular, S_none);
        painter->restore();
        if (isSelected) {
            QRect highlightRect = drawRect.adjusted(2, 1, -2, 0);
            highlightRect.setHeight(2.0);
            QRect highlightRectSpec = highlightRect.adjusted(-1, -1, 1, 0);
            painter->fillRect(highlightRectSpec, DeriveColors::lightSpecularOf(swatch.color(S_highlight)));
            painter->fillRect(highlightRect, swatch.color(S_highlight));

            QRect refillRect = rectFromInnerEdgeWithThickness(shapeClipRect, innerEdge, 2);
            refillRect = rectTranslatedTowardEdge(refillRect, innerEdge, 2);
            refillRect = expandRect(refillRect, edgeAwayNextTab | edgeTowardNextTab, -1);
            painter->fillRect(refillRect, swatch.color(tabFrameColor));
            fillRectEdges(painter, refillRect, edgeAwayNextTab | edgeTowardNextTab, 1, swatch.color(specular));
        }
        break;
    }
    case CE_ItemViewItem: {
        ...
        if (overdrawGridHack) {
            ...
            fillRectOutline(painter, r, 1, swatch.color(S_base_divider));
        }
        QCommonStyle::drawControl(element, option, painter, widget);
        break;
    }
                                   ...
                                   const QWidget* widget) const
{
    ...
    case CC_SpinBox: {
        ...
        if (!spinBox)
            break;
        const qreal rounding = SpinBox_Rounding;
        bool isLeftToRight = option->direction != Qt::RightToLeft;
        ...
        if (spinBox->frame) {
            ...
            painter->save(); // 0
            // Fill background
            paintBorderedRoundRect(painter, rect, rounding, swatch, S_none, S_base);
            // Draw button fill
            painter->setClipRect(upDownRect);
            // Side with the border
            Qt::Edge edge = isLeftToRight ? Qt::LeftEdge : Qt::RightEdge;
            paintBorderedRoundRect(
                painter, expandRect(upDownRect, oppositeEdge(edge), -1), rounding, swatch, S_none, S_button);
            painter->restore(); // 0
            if (OverhangShadows && !hasFocus && isEnabled) {
                ...
                fillRectEdges(painter, shadowRect, Qt::TopEdge, 1, swatch.color(S_base_shadow));
            }
            ...
            fillRectEdges(painter, upDownRect, edge, 1, swatch.color(S_window_outline));
            PSave save(painter);
            // Outline over entire frame
            Swatchy outlineColor = hasFocus ? S_highlight_outline : S_window_outline;
            paintBorderedRoundRect(painter, rect, rounding, swatch, outlineColor, S_none);
            save.restore();
        }
    ...
    case CC_TitleBar: {
        ...
        if ((titleBar->subControls & SC_TitleBarMinButton) && (titleBar->titleBarFlags & Qt::WindowMinimizeButtonHint)
            ...
            if (minButtonRect.isValid()) {
                ...
                drawMdiButton(painter, titleBar, minButtonRect, hover, sunken);
        ...
        if ((titleBar->subControls & SC_TitleBarMaxButton) && (titleBar->titleBarFlags & Qt::WindowMaximizeButtonHint)
            ...
            if (maxButtonRect.isValid()) {
                ...
                drawMdiButton(painter, titleBar, maxButtonRect, hover, sunken);
        ...
        if ((titleBar->subControls & SC_TitleBarCloseButton) && (titleBar->titleBarFlags & Qt::WindowSystemMenuHint)) {
            ...
            if (closeButtonRect.isValid()) {
                ...
                drawMdiButton(painter, titleBar, closeButtonRect, hover, sunken);
                QRect closeIconRect =
        ...
        if ((titleBar->subControls & SC_TitleBarNormalButton)
            ...
            if (normalButtonRect.isValid()) {
                ...
                drawMdiButton(painter, titleBar, normalButtonRect, hover, sunken);
        ...
        if (titleBar->subControls & SC_TitleBarContextHelpButton
            ...
            if (contextHelpButtonRect.isValid()) {
                ...
                drawMdiButton(painter, titleBar, contextHelpButtonRect, hover, sunken);
        ...
        if (titleBar->subControls & SC_TitleBarShadeButton && (titleBar->titleBarFlags & Qt::WindowShadeButtonHint)) {
            ...
            if (shadeButtonRect.isValid()) {
                ...
                drawMdiButton(painter, titleBar, shadeButtonRect, hover, sunken);
                drawArrow(painter, shadeButtonRect.adjusted(5, 7, -5, -7), Qt::UpArrow, swatch);
            }
        }
        ...
        if (titleBar->subControls & SC_TitleBarUnshadeButton && (titleBar->titleBarFlags & Qt::WindowShadeButtonHint)) {
            ...
            if (unshadeButtonRect.isValid()) {
                ...
                drawMdiButton(painter, titleBar, unshadeButtonRect, hover, sunken);
                drawArrow(painter, unshadeButtonRect.adjusted(5, 7, -5, -7), Qt::DownArrow, swatch);
            }
        }
    ...
    case CC_ScrollBar: {
        ...
        if (scrollBar->subControls & SC_ScrollBarSlider) {
            ...
            // paintSolidRoundRect(painter, scrollBarSlider, radius, swatch, S_scrollbarSlider);
            painter->save();
            painter->setRenderHint(QPainter::Antialiasing);
            painter->setPen(Qt::NoPen);
    ...
    case CC_ComboBox: {
        ...
        if (comboBox->editable) {
            ...
            if (comboBox->frame) {
                ...
                br.adjust(0, 1, 0, -1);
                fillRectEdges(painter, br, edge, 1, swatch.color(color));
                br.adjust(1, 0, -1, 0);
                Swatchy specular = isSunken ? S_button_pressed_specular : S_button_specular;
                fillRectOutline(painter, br, 1, swatch.color(specular));
            }
        } else {
        ...
        if (comboBox->subControls & SC_ComboBoxArrow) {
            int margin =
                static_cast<int>(qMin(downArrowRect.width(), downArrowRect.height()) * ComboBox_ArrowMarginRatio);
            QRect r = downArrowRect;
            r.adjust(margin, margin, -margin, -margin);
            // Draw the up/down arrow
            drawArrow(painter, r, Qt::DownArrow, swatch);
        }
        painter->restore();
        break;
    }
    case CC_Slider: {
        ...
        if ((option->subControls & SC_SliderGroove) && groove.isValid()) {
            ...
            paintBorderedRoundRect(painter, groove, SliderGroove_Rounding, swatch, outlineColor, gutterColor);
        }
        ...
        if ((option->subControls & SC_SliderHandle)) {
            ...
            PSave save(painter);
            paintBorderedRoundRect(painter, r, SliderHandle_Rounding, swatch, handleOutline, handleFill);
            r.adjust(1, 1, -1, -1);
            paintBorderedRoundRect(painter, r, SliderHandle_Rounding, swatch, handleSpecular, S_none);
        }
        break;
    }
    case CC_ToolButton: {
        auto tbopt = qstyleoption_cast<const QStyleOptionToolButton*>(option);
        if (AllowToolBarAutoRaise || !tbopt || !widget || !widget->parent()
...
int Style::pixelMetric(PixelMetric metric, const QStyleOption* option, const QWidget* widget) const
{
    ...
    case PM_SmallIconSize: {
        auto iconSize = QCommonStyle::pixelMetric(metric, option, widget);
        return iconSize;
    }
                                  ...
                                  const QWidget* widget) const
{
    ...
    case CT_MenuBarItem: {
        ...
        // int w = static_cast<int>(fontHeight * MenuBar_HorizontalPaddingFontRatio);
        // int h = static_cast<int>(fontHeight * MenuBar_VerticalPaddingFontRatio);
        // int line = dpiScaled(1);
    ...
    case CT_MenuItem: {
        ...
        if (UseQMenuForComboBoxPopup && qobject_cast<const QComboBox*>(widget)) {
        ...
        auto metrics = MenuItemMetrics::ofFontHeight(fontMetricsHeight);
        ...
        QSize sz;
        sz.setWidth(qMax<int>(w, MenuMinimumWidth));
        sz.setHeight(h);
        return sz;
    }
    case CT_Menu: {
        if (!MenuExtraBottomMargin || !option || !widget)
            break;
        ...
        // int vmargin = static_cast<int>(fheight * MenuItem_SeparatorHeightFontRatio) / 2;
        // QSize sz = size;
    ...
    case CT_GroupBox: {
        ...
        if (opt->subControls & (SC_GroupBoxCheckBox | SC_GroupBoxLabel)) {
            ...
            yadd += static_cast<int>(fontHeight * GroupBox_LabelBottomMarginFontRatio);
        }
...
#if 1
        if (ItemView_UseFontHeightForDecorationSize) {
    ...
    case CT_PushButton: {
        ...
        if (!pbopt || pbopt->text.isEmpty())
            break;
        int hpad = static_cast<int>(pbopt->fontMetrics.height() * PushButton_HorizontalPaddingFontHeightRatio);
        newSize.rwidth() += hpad * 2;
        if (widget && qobject_cast<const QDialogButtonBox*>(widget->parent())) {
            int dialogButtonMinWidth = 80;
    ...
    case CT_ToolButton:
        ...
        newSize += QSize(6 + DefaultFrameWidth, 6 + DefaultFrameWidth);
#elif defined(Q_OS_WIN)
        newSize += QSize(4 + DefaultFrameWidth, 4 + DefaultFrameWidth);
#else
        newSize += QSize(3 + DefaultFrameWidth, 3 + DefaultFrameWidth);
#endif
        break;
    case CT_ComboBox: {
        newSize += QSize(0, 4 + DefaultFrameWidth);
        auto cb = qstyleoption_cast<const QStyleOptionComboBox*>(option);
        ...
        if (cb) {
            int pad = 0;
            if (cb->editable) {
                pad = LineEdit_ContentsHPad;
            } else {
                pad = ComboBox_NonEditable_ContentsHPad;
            }
                                ...
                                const QWidget* widget) const
{
    ...
    case CC_SpinBox: {
        ...
        int y = fw;
        const int buttonWidth = SpinBox_ButtonWidth + 2;
        int x, lx, rx;
    ...
    case CC_GroupBox: {
        ...
        case SC_GroupBoxContents: {
            ...
            if (groupBox->subControls & (SC_GroupBoxLabel | SC_GroupBoxCheckBox)) {
                ...
                topMargin += static_cast<int>(fontHeight * GroupBox_LabelBottomMarginFontRatio);
        ...
        case SC_GroupBoxLabel: {
            ...
            int textWidth = fontMetricsWidth(option->fontMetrics, groupBox->text);
...
QRect Style::itemPixmapRect(const QRect& r, int flags, const QPixmap& pixmap) const
{
...
QStyle::SubControl Style::hitTestComplexControl(ComplexControl cc,
...
QPixmap Style::generatedIconPixmap(QIcon::Mode iconMode, const QPixmap& pixmap, const QStyleOption* opt) const
{
    ...
    if (iconMode == QIcon::Selected) {
        ...
        QColor color =
            DeriveColors::adjustLightness(opt->palette.color(QPalette::Normal, QPalette::Highlight), .25);
                         ...
                         QStyleHintReturn* returnData) const
{
    ...
    case SH_ItemView_ShowDecorationSelected:
        ...
        if (!ShowItemViewDecorationSelected && qobject_cast<const QListView*>(widget))
            return 1;
        return ShowItemViewDecorationSelected;
    ...
    case SH_ComboBox_Popup: {
        return UseQMenuForComboBoxPopup;
...
QRect Style::subElementRect(SubElement sr, const QStyleOption* opt, const QWidget* w) const
{
    switch (sr) {
